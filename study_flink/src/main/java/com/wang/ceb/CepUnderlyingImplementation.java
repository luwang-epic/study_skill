...
NFA 的原理涉及到较多数学知识，这里用一个具体的例子来说明一下状态机的工作方式，以更好地理解 CEP的原理。
 */
 ...
 * cep的底层原理
 */
public class CepUnderlyingImplementation {
    ...
    public static class StateMachineMapper extends RichFlatMapFunction<LoginEvent, String> {
        ...
        public void flatMap(LoginEvent event, Collector<String> out) throws Exception {
            ...
            if (nextState == State.Matched) {
                ...
                // 因为第三个失败事件匹配成功后，后面再来一个事件也应该匹配成功，因为前面有2个了，所以需要回到S2状态
            }
    ...
    private static class Transition implements Serializable {
        ...
        public Transition(String eventType, State targetState) {
            ...
            this.targetState = Preconditions.checkNotNull(targetState);
        }
